...
export default function WebinarDetailPage({
  ...
  if (isError || !event || event.eventType !== "WEBINAR") {
    return (
      <div className="flex min-h-[60vh] flex-col items-center justify-center bg-stone-50 text-center p-6">
        ...
        <h2 className="font-trajan text-2xl font-bold text-stone-900 tracking-wide">Webinar Not Found</h2>
        <p className="mt-4 text-stone-500 max-w-md leading-relaxed">
          The webinar you are looking for might have been removed or you do not have permission to view it.
        </p>
        <Link href="/webinars" className="mt-8">
          <Button className="rounded-xl bg-[#8b1d22] hover:bg-[#72181c] text-white px-8 uppercase tracking-widest text-xs font-semibold">
            Return to Webinars
          </Button>
        </Link>
      </div>
  ...
  const handleRegister = async () => {
    if (!session?.authenticated) {
      ...
      router.push(`/login?redirect=/webinars/${event.id}`)
      return
    }
      ...
      <Container className="max-w-6xl">
        ...
        <Breadcrumb className="mb-8">
          <BreadcrumbList className="text-xs uppercase tracking-widest text-stone-500">
            ...
            <BreadcrumbItem>
              <BreadcrumbLink asChild>
                <Link href="/webinars" className="hover:text-[#8b1d22] transition-colors">
                  Webinars
                </Link>
              </BreadcrumbLink>
            </BreadcrumbItem>
            ...
            <div className="bg-white p-8 md:p-10 border border-stone-200 shadow-sm rounded-xl">
              ...
              <div className="prose prose-stone max-w-none">
                <p className="text-stone-700 leading-loose text-base md:text-lg font-light whitespace-pre-line">
                  {event.description || "No description provided for this webinar."}
                </p>
              </div>
            </div>

            {/* Webinar Speakers Card */}
            {event.webinar?.speakers && event.webinar.speakers.length > 0 && (
              <div className="bg-white p-8 md:p-10 border border-stone-200 shadow-sm rounded-xl">
                <h3 className="font-trajan text-xl text-stone-900 mb-6 uppercase tracking-wider flex items-center gap-4">
                  Featured Speakers
                  <span className="h-0.5 flex-1 bg-[#8b1d22]/10"></span>
                ...
                <div>
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="flex gap-4">
                      ...
                      <div>
                        <p className="text-[10px] font-bold text-stone-400 uppercase tracking-widest mb-1">Access</p>
                  ...
                  {isCompleted || isCancelled ? (
                    <div className="bg-stone-50 border border-stone-200 p-6 text-center rounded-xl">
                      ...
                      <p className="text-sm font-bold uppercase tracking-widest text-stone-600">
                        {isCancelled ? "Webinar Cancelled" : "Registration Closed"}
                      </p>
                    ...
                    <div className="bg-stone-50 border border-stone-200 p-6 text-center rounded-xl">
                      ...
                      <p className="text-sm text-stone-600 mt-2 font-serif italic">
                        Your virtual seat has been reserved.
                      </p>
                      {event.meetingLink && (
                        <a
                        ...
                        >
                          <ExternalLink className="h-4 w-4" /> Join Webinar Link
                        </a>
                      )}
                    </div>
                  ) : isFullyBooked ? (
                    ...
                    <div className="space-y-4">
                      ...
                      >
                        {registering ? "Processing..." : "Register for Webinar"}
                      </Button>
                      {!session?.authenticated && (
