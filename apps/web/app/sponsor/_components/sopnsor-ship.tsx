...
export default function Sponsorship() {
  ...
  if (error) {
    return (
      <div className="flex min-h-[400px] flex-col items-center justify-center bg-white py-16 text-slate-500">
        ...
        <h2 className="text-xl font-semibold text-slate-700">Failed to load plans</h2>
        <p className="mt-2 text-sm text-slate-500">Please try refreshing the page later.</p>
      </div>
    )
  }
  ...
  return (
    <div className="bg-white py-16">
      <Container className="mx-auto">
        <h2 className="mb-12 whitespace-pre-line text-center font-trajan text-4xl font-extrabold tracking-wide text-primary">
          {t("sectionTitle")}
        </h2>
        ...
        <div className="mb-8 grid gap-8 md:grid-cols-2">
          {displayTiers.slice(0, 2).map((tier:any) => (
            ...
            >
              {tier.isFeatured && (
                <span className="absolute right-0 top-0 bg-primary px-3 py-1 text-xs uppercase text-white">
                  Most Impactful
                </span>
              )}
              ...
              <p className="mb-6 text-2xl font-bold text-primary">
                {tier.price || `${tier.currency === 'USD' ? '$' : '€'} ${tier.amount}`}{" "}
              ...
              <ul className="mb-8 flex-grow space-y-4">
                {(tier.features || tier.benefits || []).map((feat: string, i: number) => (
