...
import { CheckCircle2, Download, ArrowRight, ExternalLink, FileText, Copy, Check } from "lucide-react"
...
function DonationReceiptPDF({ amount, currency, date, txId, receiptUrl }: DonationPDFProps) {
  const currencySymbol = currency === "USD" ? "$" : "€"

  return (
    <Document title={`Donation-Receipt-${txId}.pdf`}>
      <Page size="A4" style={pdfStyles.page}>
        <View style={pdfStyles.header}>
          <Text style={pdfStyles.orgTitle}>FCH Catechesis</Text>
          <Text style={pdfStyles.receiptTitle}>Official Donation Receipt</Text>
          <Text style={pdfStyles.subtitle}>Faith, Formation, and Community Leadership</Text>
        </View>

        <View style={pdfStyles.section}>
          <Text style={pdfStyles.sectionTitle}>Donation Summary</Text>

          <View style={pdfStyles.row}>
            <Text style={pdfStyles.label}>Contribution Type</Text>
            <Text style={pdfStyles.value}>General Donation</Text>
          </View>

          <View style={pdfStyles.row}>
            <Text style={pdfStyles.label}>Amount Donated</Text>
            <Text style={pdfStyles.value}>{currencySymbol}{amount} {currency}</Text>
          </View>

          <View style={pdfStyles.row}>
            <Text style={pdfStyles.label}>Donation Date</Text>
        ...
        <View style={pdfStyles.footer}>
          <Text style={pdfStyles.footerText}>
            Thank you for your generous support. Your tax-deductible contribution aids Hispanic Catholic leadership.
          </Text>
          <Text style={pdfStyles.footerText}>
...
function ReceiptContent() {
  ...
  const rawTxId =
    ...
    "TXN-DONATION-" + Math.random().toString(36).substring(2, 8).toUpperCase()
      ...
      <CardFooter className="flex flex-col gap-3 p-8">
        ...
        {isClient ? (
          <PDFDownloadLink
            ...
            fileName={`Donation-Receipt-${txId}.pdf`}
            className="w-full"
          >
            {({ loading }) => (
              <Button
