      ...
      <div className="flex items-center justify-between space-y-2">
        <div>
          ...
          <p className="text-muted-foreground">
            Schedule new portal events, manage zoom links, and track member registrations.
          </p>
        </div>
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <ul className="mt-2 list-inside list-disc space-y-1 text-muted-foreground">
              <li>Event creation forms: Build forms to insert event details (Title, date/time, Zoom details, category, paid/free ticket details) into the events database.</li>
              <li>Registrant list audits: Implement a sub-page displaying a list of users registered for each event, with buttons to export attendee CSVs.</li>
              <li>Zoom Integration: Setup Zoom API or calendar hooks to automatically schedule Zoom conferences when creating virtual events.</li>
            </ul>
          </div>
        </div>
      </div>
        ...
        {[
          ...
          <div key={index} className="flex flex-col md:flex-row md:items-center justify-between rounded-lg border bg-card p-6 shadow-xs gap-4">
            <div className="space-y-1">
              ...
              <h4 className="font-bold tracking-tight text-base">{event.title}</h4>
              <p className="text-sm text-muted-foreground">Date: {event.date} • {event.registrants}</p>
            </div>
