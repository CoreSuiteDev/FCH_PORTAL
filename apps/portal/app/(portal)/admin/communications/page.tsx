  ...
  return (
    <div className="flex-1 space-y-6 p-8 pt-6">
      ...
      <div className="flex items-center justify-between space-y-2">
        <div>
          <h2 className="text-3xl font-bold tracking-tight">Communications Panel</h2>
          <p className="text-muted-foreground">
            Draft and send communications, newsletters, and announcements to members.
          </p>
        </div>
      </div>
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <ul className="mt-2 list-inside list-disc space-y-1 text-muted-foreground">
              <li>Email broadcast API: Setup a POST endpoint `/api/admin/communications/send` to process email broadcasts.</li>
              <li>Integrate mailing provider: Connect form fields to SendGrid, Mailchimp, or AWS SES API.</li>
              <li>User querying segmentation: Based on recipient select dropdown (All Members, Pastoral, Board), query the specific email arrays from the database.</li>
              <li>Log messages history: Save dispatched campaigns in the `communication_logs` table.</li>
            </ul>
          </div>
        </div>
      </div>
      ...
      <div className="rounded-lg border bg-card p-6 max-w-2xl space-y-4">
        ...
        <div className="space-y-3">
          <div className="space-y-1">
            <label className="text-xs font-semibold text-muted-foreground">Recipient Tier Segment</label>
          ...
          <div className="space-y-1">
            ...
            <input type="text" placeholder="e.g. FCH Resource Updates - June 2026" className="w-full rounded border p-2 text-sm bg-secondary/50 focus:outline-none" />
          </div>
          <div className="space-y-1">
            <label className="text-xs font-semibold text-muted-foreground">Email Body</label>
            <textarea placeholder="Write email text here..." rows={6} className="w-full rounded border p-2 text-sm bg-secondary/50 focus:outline-none resize-none" />
          </div>
        </div>
