...
import { EventStatsCards } from "../events/_components/event-stats-cards"
import { EventFilters } from "../events/_components/event-filters"
import { EventTable } from "../events/_components/event-table"
import { EventFormDialog } from "../events/_components/event-form-dialog"
import { EventDeleteDialog } from "../events/_components/event-delete-dialog"
...
export default function WebinarsAdminPage() {
  ...
  const displayedEvents = useMemo(() => {
    ...
    return raw.filter((ev) => {
      // Show only WEBINAR types on the Webinars page
      if (ev.eventType !== "WEBINAR") return false
  ...
  // Compute Stats
  const stats = useMemo(() => {
    const all = (eventsData?.data || []).filter((e) => e.eventType === "WEBINAR")
    return {
      total: all.length,
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400">
            Create, manage, and monitor webinars.
          </p>
        </div>
        <Button onClick={handleOpenCreate} className="h-10 hover:cursor-pointer">
          <Plus className="mr-2 size-4" /> Create Webinar
        </Button>
      </div>
