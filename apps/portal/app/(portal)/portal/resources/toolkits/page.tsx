  ...
  return (
    <div className="flex-1 space-y-6 p-8 pt-6">
      ...
      <div className="flex items-center justify-between space-y-2">
        <div>
          <h2 className="text-3xl font-bold tracking-tight">Ministry Toolkits & Guides</h2>
          <p className="text-muted-foreground">
            Outreach templates, parish checklists, coordination planners, and ministry toolkits.
          </p>
        </div>
      </div>
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <ul className="mt-2 list-inside list-disc space-y-1 text-muted-foreground">
              <li>Access verification: Check user role. General members should only see 'Limited' options; full toolkits and guides should be unlocked only for Pastoral/Board members.</li>
              <li>Setup file packaging: Create a backend route to bundle multiple toolkit files (PDF + DOCX + Excel planning charts) into a single ZIP for easy download.</li>
              <li>Provide search tagging: Tag items by event scale (Small Parish, Multi-Community, Diocese-Wide).</li>
            </ul>
          </div>
        </div>
      </div>
        ...
        {[
          ...
          <div key={index} className="flex flex-col justify-between rounded-lg border bg-card p-6 shadow-xs relative overflow-hidden">
            ...
            <div>
              ...
              <h4 className="mt-2 font-bold tracking-tight text-base">{item.title}</h4>
              <p className="mt-2 text-sm text-muted-foreground line-clamp-3">{item.desc}</p>
            </div>
