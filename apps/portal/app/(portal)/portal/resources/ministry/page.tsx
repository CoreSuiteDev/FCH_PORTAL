  ...
  return (
    <div className="flex-1 space-y-6 p-8 pt-6">
      ...
      <div className="flex items-center justify-between space-y-2">
        <div>
          <h2 className="text-3xl font-bold tracking-tight">Parish & Diocese Ministry Resources</h2>
          <p className="text-muted-foreground">
            Diocesan guidelines, pastoral policies, ministry models, and parish governance materials.
          </p>
        </div>
      </div>
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <ul className="mt-2 list-inside list-disc space-y-1 text-muted-foreground">
              <li>Role Access restrictions: Secure this route entirely. Only logged-in users with a valid `Pastoral` or `Board` membership status inside Okta groups should be permitted to view this content.</li>
              <li>Setup dynamic categories: Organize by Parish Ministry, Diocesan Policies, and Liturgical Guidelines.</li>
              <li>Implement audit logging: Keep tracks of files viewed or downloaded since diocesan documents contain policy declarations.</li>
            </ul>
          </div>
        </div>
      </div>
        ...
        {[
          ...
          <div key={index} className="flex flex-col justify-between rounded-lg border bg-card p-6 shadow-xs relative overflow-hidden">
            ...
            <div className="mt-6 pt-3 border-t">
              <button className="flex w-full items-center justify-center gap-1.5 rounded-md bg-primary px-4 py-2.5 text-xs font-semibold text-primary-foreground hover:bg-primary/90 transition-colors">
                Access Ministry Guidelines <IconChevronRight className="size-3.5" />
              </button>
            </div>
          </div>
        ))}
